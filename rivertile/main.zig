    ...
    fn layoutListener(layout: *river.LayoutV3, event: river.LayoutV3.Event, output: *Output) void {
        switch (event) {
            ...
            .layout_demand => |ev| {
                ...
                const secondary_count = @min(math.maxInt(u31), ev.view_count) -| main_count;

                // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                const usable_width = switch (output.main_location) {
                    .left, .right => @as(u31, @intCast(ev.usable_width)) -| (2 *| outer_padding),
                    .top, .bottom => @as(u31, @intCast(ev.usable_height)) -| (2 *| outer_padding),
                };
                const usable_height = switch (output.main_location) {
                    .left, .right => @as(u31, @intCast(ev.usable_height)) -| (2 *| outer_padding),
                    .top, .bottom => @as(u31, @intCast(ev.usable_width)) -| (2 *| outer_padding),
                };
